using System;
using TaskDomain.Classes;
using TaskDomain.Enums;
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                {
                    throw new Exception("You must choose valid cinema(1 or 2)");
                }
                ...
                else if (option == 2)
                {
                    ...
                    Console.WriteLine("Choose genre from the list:");
                  ...
                  //  List<string> listOfGenres = new List<string>();
                    ...
                    chosenGenre = Console.ReadLine();

                    /* bool someFlag = false;
                     foreach (string genres in listOfGenres)
                     {
                         if (genres.ToLower() == chosenGenre.ToLower())
                         {
                             someFlag = true;
                             break;
                         }
                    ...
                    bool flag = false;
                    string transformedLetterOfChosenGenre = CapitalizeFirstLetter(chosenGenre);

                    Console.WriteLine(transformedLetterOfChosenGenre);

                    if (Enum.TryParse<Genre>(transformedLetterOfChosenGenre, out Genre theGenre))//na ovoj nacin nema da funkcionira za SciFi, kje go napravi Scifi i ke padne uslovot..Najverojatno edno resenie bi bilo site mozni enum Genre da se parsiraat vo stringovi i da se stavat vo lista od stringovi i taka da se sporeduva toa shto go vnel korisnikot so elementite od listata so stringovi(so ToLower() i dvete), no pretpostavuvam deka ovoj uslov e pooptimalno resenie iako ne 100% tocno, pogore vo komentar imam napishano kako bi izgledalo na ovoj nachin
                    {
                        flag = true;
                       
                        List<Movie> listOfMoviesWithTheGenre = currentCinema.ListOfMovies.Where(x => theGenre.ToString() == transformedLetterOfChosenGenre).ToList();

                        Console.WriteLine("Choose a movie from the list:");
                        foreach (Movie movie in listOfMoviesWithTheGenre) {
                            Console.WriteLine(movie.Title);
                        }
                        string usersMovie = Console.ReadLine();


                 
                        currentCinema.WatchMovie(currentCinema.ListOfMovies.FirstOrDefault(x => x.Title.ToLower() == usersMovie.ToLower())); 

                    }
                    if (flag == false)
                    {
                        throw new Exception("Please enter a valid Genre!");
                    }

                }
                else
                {
                    throw new Exception("You must choose valid option(1 or 2)");
                }
            }
